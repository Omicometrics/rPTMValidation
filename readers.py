...
class PTMDB():
    ...
    def _add_entry(self, entry):
        ...
        for ky in PTMDB._mass_keys:
            self._data[ky].append(float(entry[ky]))
        for ky in PTMDB._name_keys:
            self._data[ky][entry[ky]] = pos
        self._data[PTMDB._desc_key][entry[ky].replace(' ', '').lower()] = pos
    ...
    def get_mass(self, name, mass_type):
        ...
        for ky in PTMDB._name_keys:
            idx = self._data[ky].get(name, None)
            if idx is not None:
...
def read_spectra_file(spec_file):
    ...
    elif spec_file.lower().endswith('.mzxml'):
...
def mzml_extract_ms1(mzml_file, ns="http://psi.hupo.org/ms/mzml"):
    """
    Extracts the MS1 spectra from the input mzML file.
    
    Args:
        msml_file (str): The path to the mzML file.
        ns (str, optional): The XML namespace used in the mzML file.
    ...
    for event, element in etree.iterparse(mzml_file, events=['end']):
        if event == 'end' and element.tag == f"{{{ns}}}spectrum":
            # This contains the cycle and experiment information
            spectrum_info = dict(element.items())
            defaultArrayLength = int(spectrum_info['defaultArrayLength'])

            # MS level
            if element.find(f"{{{ns}}}precursorList"):
                # Ignore MS level >= 2
                continue
            ...
            int_binary = element.xpath(
                ...
                namespaces=ns_map)[0]
            mz = decodebinary(mz_binary.text, defaultArrayLength)
            intensity = decodebinary(int_binary.text, defaultArrayLength)
            ...
            mz_intensity = [(mz[ii], intensity[ii])
                            for ii in range(defaultArrayLength)
                            if intensity[ii] > 0]
